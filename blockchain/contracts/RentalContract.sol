...
contract RentalContract {
    ...
    event RentalListed(uint256 indexed tokenId, address indexed owner, uint256 price, uint256 endTime);
    // Event emitted when a rental starts.
    event RentalStarted(uint256 indexed tokenId, address indexed renter);
    // Event emitted when a rental ends.
    ...
    function listForRent(
        ...
        emit RentalListed(tokenId, msg.sender, price, block.timestamp + duration);  // Emit event
    }
    ...
    // Function to end the rental after the rental period has ended.
    function endRental(uint256 tokenId) external {
        Rental storage rental = rentals[tokenId];  // Get rental info
        require(
            block.timestamp >= rental.endTime,
            "Rental period not ended yet"
        );  // Ensure rental period has ended
        rental.renter = address(0);  // Reset renter
